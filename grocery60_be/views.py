from grocery60_be import serializers
...
def record_payment(data, intent):
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            "INSERT INTO orderpayment(amount, transaction_id, payment_method, order_id, status, created_at, "
            "updated_at) VALUES (%s, %s, %s, %s, %s, %s, %s)",
            [amount, transaction_id, payment_method, order_id, status, datetime.now(), datetime.now()])
    return cursor.rowcount


def update_payment(transaction_id, status):
    with connection.cursor() as cursor:
        cursor.execute("update orderpayment set status =%s, updated_at=%s where transaction_id=%s ",
                       [amount, transaction_id, payment_method, order_id, status, datetime.now(), datetime.now()])
    return cursor.rowcount
